    ...
    <div class="bg-white shadow-sm p-4 border border-gray-200 rounded-lg">
        @if (empty($cartItems))
            <div class="py-8 text-center">
                ...
                <a href="{{ route('shop') }}" class="text-indigo-600 hover:underline">Poursuivre les achats</a>
            </div>
        @else
                            ...
                            <tr class="flex-row items-baseline gap-4 hover:bg-gray-50 border-b">
                                ...
                                <td class="py-4 text-right">
                                    <button wire:click="removeItem({{ $productId }})"
                                        class="p-1 text-gray-500 hover:text-indigo-500">
            ...
            <div class="space-y-2 mt-6 text-gray-700 text-right">
                ...
                <p class="font-semibold text-indigo-500">Livraison gratuite à partir de
            ...
            <div class="flex flex-wrap justify-between items-center gap-4 mt-6">
                <div class="flex space-x-4">
                    <a href="{{ route('shop') }}" class="text-indigo-600 hover:underline">
                        « Poursuivre les achats
                    </a>
                    <button wire:click="clearCart" class="text-gray-500 hover:text-indigo-500">
                        Vider le panier
                    </button>
                </div>
                <a href="{{ route('order') }}"
                    class="bg-indigo-600 hover:bg-indigo-300 px-6 py-2 rounded-lg text-white text-lg">
                    Commander
                </a>
            </div>
        @endif
