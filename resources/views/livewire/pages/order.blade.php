<div class="max-w-4xl mx-auto p-6">
    ...
    <div class="flex items-center mb-6">
        <div class="w-1/3 text-center">
            <div class="font-semibold p-2 border-b-4 {{ $step === 1 ? 'border-red-600 text-red-600' : 'border-gray-200 text-gray-500' }}">
                <i class="fas fa-id-card mr-1"></i>Détails
            </div>
        </div>
        <div class="w-1/3 text-center">
            <div class="font-semibold p-2 border-b-4 {{ $step === 2 ? 'border-red-600 text-red-600' : 'border-gray-200 text-gray-500' }}">
                <i class="fas fa-credit-card mr-1"></i>Mode de paiement
            </div>
        </div>
        <div class="w-1/3 text-center">
            <div class="font-semibold p-2 border-b-4 {{ $step === 3 ? 'border-red-600 text-red-600' : 'border-gray-200 text-gray-500' }}">
    ...
    @if ($step === 1)
        @if (!Auth::check())
            ...
            <div class="mb-6 space-y-4">
                <label class="flex items-center space-x-2 border p-4 rounded-md {{ $accountOption === 'register' ? 'border-red-500' : 'border-gray-200' }}">
                    <input type="radio" wire:model.live="accountOption" value="register" />
                    <span>Créer un compte</span>
                </label>

                <label class="flex items-center space-x-2 border p-4 rounded-md {{ $accountOption === 'login' ? 'border-red-500' : 'border-gray-200' }}">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" wire:model="first_name" class="border w-full px-3 py-2 bg-gray-100 rounded">
                            @error('first_name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-gray-700">Nom</label>
                            <input type="text" wire:model="last_name" class="border w-full px-3 py-2 bg-gray-100 rounded">
                    ...
                    <div>
                        <label class="block text-gray-700">Mot de passe</label>
                        <input type="password" wire:model="password" class="w-full border px-3 py-2 bg-gray-100 rounded">
                        @error('password') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-gray-700">Confirmer le mot de passe</label>
                        <input type="password" wire:model="password_confirmation" class="w-full border px-3 py-2 bg-gray-100 rounded">
                    </div>
                </div>
            @endif
        @else
            ...
            @if($use_different_shipping)
                <div class="mt-4 p-4 border rounded-lg space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-gray-700">Prénom</label>
                            <input type="text" wire:model="shipping_first_name" class="border w-full px-3 py-2 bg-gray-100 rounded">
                            @error('shipping_first_name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-gray-700">Nom</label>
                            <input type="text" wire:model="shipping_last_name" class="border w-full px-3 py-2 bg-gray-100 rounded">
                    ...
                    <div>
                        <label class="block text-gray-700">Adresse</label>
                        <input type="text" wire:model="shipping_address" class="w-full border px-3 py-2 bg-gray-100 rounded">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-gray-700">Code postal</label>
                            <input type="text" wire:model="shipping_postal_code" class="border px-3 py-2 bg-gray-100 rounded">
                            @error('shipping_postal_code') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-gray-700">Ville</label>
                            <input type="text" wire:model="shipping_city" class="border px-3 py-2 bg-gray-100 rounded">
        ...
        <div class="flex justify-between mt-6">
            <a href="{{ route('cart') }}" class="text-red-600 hover:underline">« Panier</a>
            <button wire:click="goToNextStep" class="bg-red-600 text-white px-6 py-2 rounded hover:bg-red-700">Suivant »</button>
        </div>
    ...
    @elseif ($step === 2)
        <div class="space-y-6">
            ...
            <div class="space-y-4">
                <label class="flex items-center p-4 border rounded-lg border-red-500 bg-red-50">
                    <input type="radio" wire:model="payment_method" value="transfer" class="mr-3" checked>
                    <div>
                        <span class="font-medium">Virement bancaire</span>
                        <p class="text-sm text-gray-500">Effectuez un virement sur notre compte bancaire</p>
                    </div>
                </label>

                <label class="flex items-center p-4 border rounded-lg border-gray-200 opacity-50 cursor-not-allowed">
                    <input type="radio" disabled class="mr-3">
                    <div>
                        <span class="font-medium">Carte bancaire</span>
                        <p class="text-sm text-gray-500">Temporairement indisponible</p>
                    </div>
                </label>
            ...
            <!-- Navigation -->
            <div class="flex justify-between mt-6">
                <button wire:click="goToPreviousStep" class="text-red-600 hover:underline">« Retour</button>
                <button wire:click="goToNextStep" class="bg-red-600 text-white px-6 py-2 rounded hover:bg-red-700">Vérifier la commande »</button>
            </div>
        </div>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                @if($account)
                    <div class="bg-yellow-50 border border-yellow-200 p-4 rounded-lg mb-4">
                        ...
                        <p class="mb-1"><span class="font-medium">Bénéficiaire:</span> {{ $account->owner }}</p>
                        ...
                        @endif
                        <p class="mt-3 text-sm text-yellow-700">Veuillez effectuer le virement et télécharger une preuve ci-dessous.</p>
                    </div>
                @else
                    <p class="text-red-500">Les coordonnées bancaires ne sont pas disponibles. Veuillez nous contacter.</p>
                @endif
                ...
                <div class="mt-4">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                        @if($has_proof)
                            <p class="text-green-600 mb-2"><i class="fas fa-check-circle mr-2"></i>Justificatif téléchargé</p>
                            <button wire:click="$set('has_proof', false)" class="text-sm text-red-600 hover:underline">
                                Changer de fichier
                            </button>
                        @else
                            <label for="payment_proof" class="block cursor-pointer">
                                <i class="fas fa-upload text-gray-400 text-xl mb-2"></i>
                                <p class="text-sm text-gray-500 mb-1">Cliquez pour télécharger un justificatif de paiement</p>
