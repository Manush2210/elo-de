<div class="bg-white shadow-md mx-auto mt-10 p-6 rounded-lg max-w-md">
    ...
    @if (session()->has('message'))
        <div class="bg-teal-100 mb-4 p-3 rounded text-teal-700">
            {{ session('message') }}
        </div>
    @endif
    ...
    <form wire:submit.prevent="login">
        <div class="mb-4">
            ...
            <input type="email" id="email" wire:model="email"
                class="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 w-full">
            @error('email')
        ...
        <div class="flex items-center mb-6">
            <input type="checkbox" id="remember" wire:model="remember" class="w-4 h-4 text-teal-600">
    ...
    <div class="mt-6 text-center">
        <a href="#" class="text-teal-600 text-sm hover:underline">Mot de passe oublié?</a>
        <p class="mt-2 text-gray-600">
            Pas encore de compte?
            <a href="{{ route('register') }}" class="text-teal-600 hover:underline">S'inscrire</a>
        </p>
    </div>
</div>
