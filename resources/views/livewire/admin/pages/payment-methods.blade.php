<div class="mx-auto px-4 py-8 container">
        ...
        <div class="flex md:flex-row flex-col gap-3 mt-4 md:mt-0">
            <div class="relative">
                <input type="text" wire:model.live="search" placeholder="Rechercher..."
                    class="bg-gray-800 px-4 py-2 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full md:w-64 text-white">
            ...
            <button wire:click="$dispatch('openModal')"
                class="inline-flex items-center bg-indigo-500 hover:bg-indigo-600 active:bg-indigo-700 px-4 py-2 border focus:border-indigo-700 border-transparent rounded-md focus:outline-none focus:ring focus:ring-indigo-200 font-semibold text-white text-xs uppercase tracking-widest transition duration-150 ease-in-out">
    ...
    @if (session()->has('message'))
        <div class="bg-indigo-500 mb-4 p-4 rounded-md text-white">
            {{ session('message') }}
        </div>
    @endif
            ...
            <div class="bg-gray-800 rounded-lg w-full overflow-hidden">
                <div class="p-4">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <button wire:click="deletePaymentMethod({{ $method->id }})"
                            onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce moyen de paiement ?')"
                            class="flex-1 bg-indigo-500 hover:bg-indigo-600 px-4 py-2 rounded-md text-white text-sm">
                            Supprimer
                        </button>
                    </div>
                </div>
